...
module EveData
  class EcPriceData < ActiveRecord::Base
    ...
    belongs_to :type,             class_name: 'Type', foreign_key: :typeID, primary_key: :typeID
    belongs_to :solarsystem,          class_name: 'SolarSystem', foreign_key: :solarSystemID, primary_key: :solarSystemID
    ...
    class << self
      ...
      def update_standard_market_groups(uopts = {})
        ...
        mgs.each do |mg|
          x = MarketGroup.find_by_marketGroupName(mg)
          printf("check mg: %s [%d]\n", mg, x.types.length) unless opts[:silent]
          x.ec_update(opts)
        end
        cats.each do |cat|
          x = Category.find_by_categoryName(cat)
          printf("check cat: %s [%d]\n", cat, x.types.length) unless opts[:silent]
          x.ec_update(opts)
        end
        nil
      end
